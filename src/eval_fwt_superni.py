...
# at,t 在计算bwt的时候已经有了，a0,t是fwt这里计算的
...
def get_jga_scores(output_dir, dataset_order, name_flag = False):
    ...
    for service_id in range(0, len(dataset_order)):
        #print(service_id)
        if name_flag:
            result_file = os.path.join(output_dir, dataset_order[service_id] +"_result.txt")        
        else:    
...
def main(args):
    ...
    avgPerf_list1 = get_jga_scores(output_dir, dataset_order) 
    ...
    avgPerf_list = [avgPerf_list1[i]-avgPerf_list2[i] for i in range(len(avgPerf_list1))]
    print(avgPerf_list)
    print(f"average FWT is {sum(avgPerf_list) / len(avgPerf_list)}")
    print()
...
if __name__=='__main__':
    ...
    #model_name = "Llama-2-7b-chat-hflora"
    #model_name = "qwen3"
    ...
    method_name = f"SuperNI_Ours_inner_step_{inner_step}_outer_step_{outer_step}_coldstartstep_3"
    ...
    parser.add_argument("--test_data_name2", type=str, default=model_name+"_fwt", help = "-averaging")
